...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      const deployedNetwork = MyToken.networks[this.networkId];
      const deployedNetworkII = MyTokenSale.networks[this.networkId];
      const deployedNetworkIII = KycContract.networks[this.networkId];
      ...
      this.myTokenSale = new this.web3.eth.Contract(
        MyTokenSale.abi,
        deployedNetworkII && deployedNetworkII.address,
      );

      this.kycContract = new this.web3.eth.Contract(
        KycContract.abi,
        deployedNetworkIII && deployedNetworkIII.address,
      );

      // Set web3, accounts, and contract to the state, and then proceed with an
      // example of interacting with the contract's methods.
      this.listenToTokenTransfer();
      this.setState({ loaded:true, tokenSaleAddress:deployedNetworkII.address }, this.updateUserTokens);
    } catch (error) {
  ...
  updateUserTokens = async() => {
    ...
    this.setState({userTokens: userTokens != null ? userTokens : 0});
  }
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <p>You have: {this.state.userTokens} token</p>
